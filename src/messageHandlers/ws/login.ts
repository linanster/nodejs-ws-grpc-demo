...
export default function handlerLogin(ws: WebSocket, data: any) {
  ...
  // 调用 gRPC 服务进行认证
  client.login({ username: data.username, password: data.password }, (err: any, response: any) => {
    if (err) {
      console.error('gRPC error:', err);
      ws.send(JSON.stringify({ messageType: 'loginResponse', body: { success: false, message: '认证失败' } }));
    } else {
      console.log('gRPC response:', response);
      ws.send(JSON.stringify({ messageType: 'loginResponse', body: response }));
    }
  });
}