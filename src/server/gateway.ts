...
const handlers: Record<string, (ws: WebSocket, data: any) => void> = fs.readdirSync(handlersPath)
  ...
  ws.on('message', (message) => {
    try {
      ...
      if (handler) {
        ...
        ws.send(JSON.stringify({ messageType: data.messageType, body: '暂不支持!' }));
      }
    } catch (error) {
      console.error('Invalid JSON message received:', error);
      ws.send(JSON.stringify({ messageType: 'error', body: 'Invalid JSON format' }));
    }
  });
